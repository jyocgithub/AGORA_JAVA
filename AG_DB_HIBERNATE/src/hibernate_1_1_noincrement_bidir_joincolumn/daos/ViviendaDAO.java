package hibernate_1_1_noincrement_bidir_joincolumn.daos;

import hibernate_1_1_noincrement_bidir_joincolumn.pojos.Direccion;
import hibernate_1_1_noincrement_bidir_joincolumn.pojos.Vivienda;
...
public class ViviendaDAO {
	...
	public Direccion consultarDireccion(int id) {
		...
		try {
			tx = session.beginTransaction();
			Direccion objetoencontrado = session.get(Direccion.class, id);
			tx.commit();
			return objetoencontrado;
		} catch (HibernateException e) {
			if (tx != null) tx.rollback();
