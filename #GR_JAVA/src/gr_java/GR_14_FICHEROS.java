...
public class GR_14_FICHEROS {
    ...
    public void ejemplosFilter() throws IOException {

        // **************************************
        // FILEFILTER
        // **************************************

        //        FileFilter es una interfaz funcional que permite filtrar archivos dentro de un directorio según algún criterio, como extensión, tamaño, nombre, etc.
        //       Pertenece al paquete java.io y se usa principalmente con el método:
        //        File[] archivos = directorio.listFiles(FileFilter filtro);

        // Ejemplo: Filtrar solo archivos .txt

        File carpeta = new File("C:/misArchivos");

        // asi como clase anonima interna, pero por supuesto, se puede hacer con clase propia
        // que implemente FileFilter

        File[] archivosTxt = carpeta.listFiles(new FileFilter() {
            @Override
        ...
        // Lo mismo con una expresión lambda (Java 8+) (pues es una interfaz funcional)
        File[] archivosTxt2 = carpeta.listFiles(file -> file.getName().endsWith(".txt"));

        // Solo directorios

        File[] subcarpetas = carpeta.listFiles(file -> file.isDirectory());
        ...
        // igualito que FILeFIlter, pero no devuelve un array de FIles, sino de String,
        // y se filtra solo por el nombre del ficheros, no por otras caracteristicas

        String[] archivosJPG = carpeta.list(new FilenameFilter() {
            @Override
        ...
        for (String nombre : archivosJPG) {
            System.out.println(nombre);
        }

        //        O con lambda
        String[] archivosJPG2 = carpeta.list((dir, name) -> name.toLowerCase().endsWith(".jpg"));
